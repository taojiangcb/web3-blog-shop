...
function Swap() {
  ...
  // 代币数量状态管理
  const [tokenOneAmount, setTokenOneAmount] = useState(null); // 输入代币数量
  const [tokenTwoAmount, setTokenTwoAmount] = useState(null); // 输出代币数量
  ...
  // 价格和交易状态管理
  const [prices, setPrices] = useState(null); // 存储代币对的价格数据
  ...
  function modifyToken(i: number) {
    ...
    if (changeToken === 1) {
      ...
      fetchPrices(tokenList[i] as unknown as Token, tokenTwo as unknown as Token);
    } else {
      setTokenTwo(tokenList[i]);
      fetchPrices(tokenOne as unknown as Token, tokenList[i] as unknown as Token);
    }
    setIsOpen(false);
  }
  ...
  async function approveToken(tokenAddress: string, amount: number) {
    console.log('approve token called token: +' + tokenAddress + ' amount: ' + amount);
    const tokenABI = [
      {
        inputs: [
  ...
  useEffect(() => {
    fetchPrices(tokenList[0] as unknown as Token, tokenList[1] as unknown as Token);
  }, []);
